...
let s:green_str = s:colors.green
...
if !exists('g:aranda_bold')
  let g:aranda_bold = 1
endif

if !exists('g:aranda_italic')
  let g:aranda_italic = 1
endif

if !exists('g:aranda_underline')
  let g:aranda_underline = 1
endif

if !exists('g:aranda_undercurl') && g:aranda_underline != 0
  let g:aranda_undercurl = 1
endif

if !exists('g:aranda_inverse')
  let g:aranda_inverse = 1
endif

let s:styles = {
      \ 'bold':      g:aranda_bold == 1 ? 'bold' : 0,
      \ 'italic':    g:aranda_italic == 1 ? 'italic' : 0,
      \ 'underline': g:aranda_underline == 1 ? 'underline' : 0,
      \ 'undercurl': g:aranda_undercurl == 1 ? 'undercurl' : 0,
      \ 'inverse':   g:aranda_inverse == 1 ? 'inverse' : 0,
      \ }
...
call s:hi('CursorLine',   { 'bg': s:black, 'styles': [s:styles.underline] })
...
call s:hi('Visual',       { 'fg': s:black, 'bg': s:blue })
...
call s:hi('pythonComment',       { 'fg': s:purple, 'styles': [s:styles.italic] })
call s:hi('pythonTodo',          { 'fg': s:blur_grey, 'bg': s:yellow })
call s:hi('pythonString',        { 'fg': s:green_str, 'styles': [s:styles.italic] })
call s:hi('pythonRawString',     { 'fg': s:yellow, 'styles': [s:styles.italic] })
call s:hi('pythonNumber',        { 'fg': s:blue })
